...
constexpr char presenceScoreTag[] = "PRESENCE_SCORE";
constexpr char handenessTag[] = "HANDENESS";
    ...
    CalculatorContract *cc)
{
    ...
    RET_CHECK(cc->Inputs().HasTag(presenceScoreTag));
    cc->Inputs().Tag(presenceScoreTag).Set<float>();

    RET_CHECK(cc->Inputs().HasTag(handenessTag));
    cc->Inputs().Tag(handenessTag).Set<ClassificationList>();
    ...
    CalculatorContext *cc)
{
    ...
    float score = cc->Inputs().Tag(presenceScoreTag).Get<float>();
 
    if(score<0.9){
        LOG(INFO) << "No hand detected (score:"<<score<<")";
        return ::mediapipe::OkStatus();
    }
    

    // const auto handeness = cc->Inputs().Tag(handenessTag).Get<mediapipe::ClassificationList>();
 
  const ClassificationList& classifications =
        cc->Inputs().Tag(handenessTag).Get<ClassificationList>();
    ...
    for (int i = 0; i < classifications.classification_size(); ++i) {
      LOG(INFO) << classifications.classification(i).label()<<" | score:"<< classifications.classification(i).score();

      if(classifications.classification(i).label()=="Left"){
        left_score=classifications.classification(i).score();
      }
      if(classifications.classification(i).label()=="Right"){
        right_score=classifications.classification(i).score();
      }
    }
    ...
    float distance = this->get_Euclidean_DistanceAB(landmarkList.landmark(4).x(), landmarkList.landmark(4).y(), landmarkList.landmark(17).x(), landmarkList.landmark(17).y());

    LOG(INFO)<<"teste:"<< distance; 

    

    float pseudoFixKeyPoint = landmarkList.landmark(2).x();
    // if ( (!isLeft && landmarkList.landmark(3).x() < pseudoFixKeyPoint && landmarkList.landmark(4).x() < pseudoFixKeyPoint) || (isLeft && landmarkList.landmark(3).x() > pseudoFixKeyPoint && landmarkList.landmark(4).x() > pseudoFixKeyPoint))
    if(distance>=0.5)
    {
        // LOG(INFO) << "THUMB is open";
        thumbIsOpen = true;
    }
    ...
    {
         LOG(INFO) << "5 fresquinhas para o staff da DTek";
        recognized_hand_gesture = new std::string("FIVE");
    }
    else if (!thumbIsOpen && firstFingerIsOpen && secondFingerIsOpen && thirdFingerIsOpen && fourthFingerIsOpen)
    {
         LOG(INFO) << "4 fresquinhas para o staff da DTek";
        recognized_hand_gesture = new std::string("FOUR");
    }
    else if ( secondFingerIsOpen && thirdFingerIsOpen && fourthFingerIsOpen)
    {
         LOG(INFO) << "3 fresquinhas para o staff da DTek";
        recognized_hand_gesture = new std::string("THREE");
    }
    else if (firstFingerIsOpen && secondFingerIsOpen)
    {
         LOG(INFO) << "2 fresquinhas para o staff da DTek";
        recognized_hand_gesture = new std::string("TWO");
    }
    else if (!thumbIsOpen && firstFingerIsOpen && !secondFingerIsOpen && !thirdFingerIsOpen && !fourthFingerIsOpen)
    {
        LOG(INFO) << "1 fresquinhas para o staff da DTek";
